...
用於監控與分析網路流量的系統，可即時偵測可疑行為與潛在威脅，並支援異常流量偵測，強化網路資安的防禦與可視性。
...
- **負責項目**
  - 主導前端開發 (Next.js)，並參與 UI 設計 (Figma)
  - 撰寫與調整部分後端 API (FastAPI、OpenSearch Query DSL)
...
用於監控端點裝置行為的系統，可即時偵測潛在威脅，如惡意程式、異常行為與 CVE 等，並支援事件記錄與日誌分析，強化端點資安的防禦與可視性。

- **成果**
  - 技術轉移至台灣大哥大

- **負責項目**
  - 主導前端開發 (Next.js)，並負責 UI 設計 (Figma)
...
一款結合 YouTube Music 的猜歌小遊戲。玩家可依歌手與標籤篩選歌曲池，並可新增、刪除歌曲或編輯標籤 (目前僅作者帳號可異動歌曲與標籤)。
